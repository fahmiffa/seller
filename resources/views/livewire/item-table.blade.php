    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
        <div class="mb-4 flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="w-full md:w-1/2 flex gap-2">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama produk atau jasa..."
                ...
                @if($search)
                <button wire:click="$set('search', '')" class="flex items-center text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                    Reset
                </button>
                @endif
            </div>
            <div class="flex flex-wrap gap-2 w-full md:w-auto items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('items.print-qrcode', ['search' => $search]) }}" target="_blank"
                        class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg transition shadow-md flex items-center gap-2 text-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('items.create') }}" wire:navigate
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition shadow-md shadow-blue-500/20 flex items-center gap-2 text-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr wire:key="item-{{ $item->item_id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="bg-white p-1 inline-block rounded shadow-sm border border-gray-100">
                                        {!! QrCode::size(40)->generate($item->item_id . '-' . $item->user_id) !!}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <button type="button" @click="confirmDelete('{{ route('items.destroy', $item) }}')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">Hapus</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="8" class="px-6 py-4 text-center text-gray-500">Tidak ada data produk/jasa.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
