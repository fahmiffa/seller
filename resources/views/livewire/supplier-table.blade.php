<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
    <div class="mb-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div class="flex justify-end w-full md:w-auto">
            <a href="{{ route('suppliers.create') }}" wire:navigate class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition ease-in-out duration-150 text-sm">
                Tambah Supplier
            </a>
        </div>
    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($suppliers as $supplier)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $supplier->nama_supplier }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $supplier->telepon }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $supplier->email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ Str::limit($supplier->alamat, 30) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <button type="button" onclick="confirmDelete('{{ route('suppliers.destroy', $supplier) }}')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">Hapus</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500 text-sm">Tidak ada data supplier.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
