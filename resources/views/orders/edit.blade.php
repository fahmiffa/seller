<x-app-layout>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('orders.update', $order) }}" method="POST" id="orderForm">
                        ...
                        @if(auth()->user()->role == 4)
                            ...
                            <!-- Distributor (Role 4): Verifikasi / Pilih Supplier -->
                            <div class="mb-4">
                                ...
                                <select name="supplier_id" id="supplier_id" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('supplier_id') border-red-500 @enderror" required>
                                    ...
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->supplier_id }}" {{ old('supplier_id', $order->supplier_id) == $supplier->supplier_id ? 'selected' : '' }}>
                        ...
                        @else
                            <!-- Unit (Role 5): Info Status -->
                            <div class="mb-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md border border-gray-200 dark:border-gray-600">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Status Order: </span>
                                <span class="text-sm font-semibold capitalize {{ $order->status == 'pending' ? 'text-yellow-600 dark:text-yellow-400' : 'text-blue-600 dark:text-blue-400' }}">{{ $order->status }}</span>
                            </div>
                        @endif
                        ...
                        <div class="mb-4">
                            ...
                            <div id="items-container" class="space-y-4">
                                <!-- Existing item rows will be populated by JS -->
                            </div>
                        </div>

                        @if(auth()->user()->role == 4)
                            <!-- Distributor (Role 4): Status -->
                            <div class="mb-4">
                                ...
                                <select name="status" id="status" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('status') border-red-500 @enderror" required>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="date" name="tanggal_po" id="tanggal_po" value="{{ old('tanggal_po', $order->tanggal_po ? $order->tanggal_po->format('Y-m-d') : date('Y-m-d')) }}"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3"
                                ...
                                placeholder="Catatan tambahan order...">{{ old('keterangan', $order->keterangan) }}</textarea>
                            @error('keterangan')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        // Existing order items from the database
        @php
            $mappedItems = $order->items->map(function($item) {
                return [
        ...
        @endphp
        var existingItems = @json($mappedItems);
            ...
            const html = `
                <div class="item-row bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-600 rounded-lg p-4" data-index="${idx}">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button type="button" onclick="removeItem(this)" class="text-red-500 hover:text-red-700 text-sm font-medium transition">
        ...
        function removeItem(button) {
            ...
            if (container.children.length <= 1) {
                alert('Order harus memiliki minimal 1 komoditas.');
                return;
            }
